...
async function main() {
  log('\n🔧 WarpCMS Database Reset Tool', colors.cyan + colors.bold);
  log('================================\n', colors.cyan);
  ...
  if (!wranglerInfo) {
    ...
    log('Please run this command from your WarpCMS project directory.', colors.yellow);
    process.exit(1);
  }
  ...
  const dbName = `warpcms-worktree-${safeBranch}`;
