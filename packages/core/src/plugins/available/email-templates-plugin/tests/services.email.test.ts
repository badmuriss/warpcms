    ...
    describe('sendTemplatedEmail', () => {
      it('should send templated email successfully', async () => {
        ...
        const result = await emailService.sendTemplatedEmail({
          ...
          variables: { name: 'John', company: 'WarpCMS' },
        ...
        expect(mockRenderTemplate).toHaveBeenCalledWith(
          'welcome-email',
          { name: 'John', company: 'WarpCMS' }
        )

        // Verify email was sent with rendered content
        const callArgs = vi.mocked(fetch).mock.calls[0]
        const requestBody = JSON.parse(callArgs[1].body as string)
        expect(requestBody.personalizations[0].subject).toBe('Welcome to WarpCMS')
