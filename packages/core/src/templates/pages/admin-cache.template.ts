 ...
 *
 * Moved from @warpcms/templates to avoid circular dependency
...
export function renderCacheDashboard(data: CacheDashboardData): string {
  ...
  const pageContent = `
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-semibold text-zinc-950 dark:text-white">${t('cache.title', locale)}</h1>
          <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
            ${t('cache.subtitle', locale)}
          </p>
        </div>
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ${renderStatCard(t('cache.totalRequests', locale), data.totals.requests.toLocaleString(), 'lime', `
          <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/>
          </svg>
        `)}

        ${renderStatCard(t('cache.hitRate', locale), data.totals.hitRate + '%', 'blue', `
          <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
          </svg>
        `, parseFloat(data.totals.hitRate) > 70 ? 'lime' : parseFloat(data.totals.hitRate) > 40 ? 'amber' : 'red')}

        ${renderStatCard(t('cache.memoryUsage', locale), formatBytes(data.totals.memorySize), 'purple', `
          <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z"/>
          </svg>
        `)}

        ${renderStatCard(t('cache.cachedEntries', locale), data.totals.entryCount.toLocaleString(), 'sky', `
          <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="overflow-hidden rounded-xl bg-white dark:bg-zinc-900 ring-1 ring-zinc-950/5 dark:ring-white/10">
        <div class="px-6 py-4 border-b border-zinc-950/5 dark:border-white/10">
          <h2 class="text-lg font-semibold text-zinc-950 dark:text-white">${t('cache.cacheNamespaces', locale)}</h2>
        </div>
          ...
          <table class="min-w-full divide-y divide-zinc-950/5 dark:divide-white/10">
            <thead class="bg-zinc-50 dark:bg-zinc-800/50">
              <tr>
                <th class="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">
                  ${t('cache.namespace', locale)}
                </th>
                <th class="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">
                  ${t('cache.requests', locale)}
                </th>
                <th class="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">
                  ${t('cache.hitRate', locale)}
                </th>
                <th class="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">
                  ${t('cache.memoryHits', locale)}
                </th>
                <th class="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">
                  ${t('cache.kvHits', locale)}
                </th>
                <th class="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">
                  ${t('cache.entries', locale)}
                </th>
                <th class="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">
                  ${t('cache.size', locale)}
                </th>
                <th class="px-6 py-3 text-right text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">
                  ${t('cache.actions', locale)}
                </th>
              </tr>
            </thead>
            <tbody class="divide-y divide-zinc-950/5 dark:divide-white/10">
              ${data.namespaces.map(namespace => {
                ...
                if (!stat) return ''
                return renderNamespaceRow(namespace, stat, locale)
              }).join('')}
            </tbody>
          </table>
        </div>
      ...
      <div class="overflow-hidden rounded-xl bg-white dark:bg-zinc-900 ring-1 ring-zinc-950/5 dark:ring-white/10">
        <div class="px-6 py-4 border-b border-zinc-950/5 dark:border-white/10">
          <h2 class="text-lg font-semibold text-zinc-950 dark:text-white">${t('cache.performanceOverview', locale)}</h2>
        </div>
        <div class="p-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ${renderPerformanceMetric(t('cache.memoryCache', locale), data.totals.hits, data.totals.misses, locale)}
            ${renderHealthStatus(parseFloat(data.totals.hitRate), locale)}
          </div>
        </div>
      </div>
    </div>
    ...
    <script>
      window.__i18n = {
        allCachesClearedSuccess: '${t('cache.allCachesClearedSuccess', locale)}',
        errorClearingCaches: '${t('cache.errorClearingCaches', locale)}',
        cacheClearedSuccess: '${t('cache.cacheClearedSuccess', locale)}',
        errorClearingCache: '${t('cache.errorClearingCache', locale)}',
      };
      ...
      async function performClearAllCaches() {
        try {
          ...
          if (result.success) {
            alert(window.__i18n.allCachesClearedSuccess)
            window.location.reload()
          } else {
            alert(window.__i18n.errorClearingCaches + ': ' + result.error)
          }
        } catch (error) {
          alert(window.__i18n.errorClearingCaches + ': ' + error.message)
        }
      }
      ...
      async function performClearNamespaceCache() {
        ...
        try {
          ...
          const result = await response.json()
          if (result.success) {
            alert(window.__i18n.cacheClearedSuccess)
            window.location.reload()
          } else {
            alert(window.__i18n.errorClearingCache + ': ' + result.error)
          }
        } catch (error) {
          alert(window.__i18n.errorClearingCache + ': ' + error.message)
        } finally {
    ...
    ${renderConfirmationDialog({
      ...
      title: t('cache.clearAllCacheTitle', locale),
      message: t('cache.clearAllCacheMessage', locale),
      confirmText: t('cache.clearAll', locale),
    ...
    ${renderConfirmationDialog({
      ...
      title: t('cache.clearNamespaceCacheTitle', locale),
      message: t('cache.clearNamespaceCacheMessage', locale),
      confirmText: t('cache.clear', locale),
      cancelText: t('common.cancel', locale),
      iconColor: 'yellow',
      confirmClass: 'bg-yellow-500 hover:bg-yellow-400',
  ...
  const layoutData: AdminLayoutCatalystData = {
    title: t('cache.title', locale),
    pageTitle: t('cache.title', locale),
...
function renderNamespaceRow(namespace: string, stat: CacheStats, locale = 'en'): string {
...
function renderPerformanceMetric(label: string, hits: number, misses: number, locale = 'en'): string {
    ...
    <div>
      ...
      <div class="space-y-2">
        <div class="flex items-center justify-between text-sm">
          <span class="text-zinc-600 dark:text-zinc-400">${t('cache.hits', locale)}</span>
          <span class="font-medium text-zinc-900 dark:text-zinc-100">${hits.toLocaleString()}</span>
        </div>
        <div class="flex items-center justify-between text-sm">
          <span class="text-zinc-600 dark:text-zinc-400">${t('cache.misses', locale)}</span>
          <span class="font-medium text-zinc-900 dark:text-zinc-100">${misses.toLocaleString()}</span>
        </div>
        <div class="mt-3">
          <div class="flex items-center justify-between text-sm mb-1">
            <span class="text-zinc-600 dark:text-zinc-400">${t('cache.hitRate', locale)}</span>
...
function renderHealthStatus(hitRate: number, locale = 'en'): string {
  const status = hitRate > 70 ? 'healthy' : hitRate > 40 ? 'warning' : 'critical'
  const statusConfig = {
    healthy: {
      label: t('cache.healthy', locale),
      color: 'lime',
    ...
    warning: {
      label: t('cache.needsAttention', locale),
      color: 'amber',
      icon: `<svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return `
    <div>
      <h3 class="text-sm font-medium text-zinc-900 dark:text-zinc-100 mb-3">${t('cache.systemHealth', locale)}</h3>
      <div class="flex items-center gap-3 p-4 rounded-lg ring-1 ring-inset ${colorClasses[config.color as keyof typeof colorClasses]}">
        ...
        <div>
          ...
          <p class="text-xs mt-0.5 opacity-80">
            ${status === 'healthy' ? t('cache.healthyMessage', locale) :
              status === 'warning' ? t('cache.warningMessage', locale) :
              t('cache.criticalMessage', locale)}
          </p>
        </div>
      </div>
    </div>
