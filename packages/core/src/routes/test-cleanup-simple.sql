...
DELETE FROM api_tokens WHERE user_id IN (
  SELECT id FROM users WHERE email != 'admin@warpcms.com' AND (email LIKE '%test%' OR email LIKE '%example.com%')
);
...
DELETE FROM users WHERE email != 'admin@warpcms.com' AND (email LIKE '%test%' OR email LIKE '%example.com%');
